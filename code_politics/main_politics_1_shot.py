...
LastEditors: zrk
LastEditTime: 2024-06-15 02:47:17
'''
import openai
import jsonlines
import os
...
fh = logging.FileHandler(os.path.join(opt.save_file, 'log_politics_p5_temperature{}_1-shot.txt'.format(opt.temperature)))
log.addHandler(fh)
...
def form_template_label(test_data):
    ...
    for data in test_data:
        ...
        # prompt_template = "La position de " + comment + " envers " + question + " est? Give me a one-word answer. Select from 'Favor' and 'Against'."
        # p5
        prompt_template = "La position de " + comment + " envers " + question + " est [MASK]. Select from 'Favor' and 'Against'."
...
def main():
    ...
    for sub in sub_datasets_list:
        ...
        for i, test_template in enumerate(test_templates[opt.startpoint:], start=opt.startpoint):
            ...
            predict_word = send_request_1_shot(prompt_template, example_template, example_answer) 
