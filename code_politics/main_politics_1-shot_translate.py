...
LastEditors: zrk
LastEditTime: 2024-07-30 01:57:30
'''
import openai
import jsonlines
import os
...
fh = logging.FileHandler(os.path.join(opt.save_file, 'log_politics_p5_temperature{}_translated_1-shot_{}_1.txt'.format(opt.temperature, opt.index1)))
log.addHandler(fh)
...
def get_1_example(dataset, index1):
    example_templates = []
...
def send_request_1_shot(template, example_templates):
...
def main():
    ...
    for sub in sub_datasets_list:
        ...
        example_templates = get_1_example(de_train, opt.index1)
        ...
        for i, test_template in enumerate(test_templates[opt.startpoint:], start=opt.startpoint):
            ...
            predict_word = send_request_1_shot(prompt_template, example_templates) 
